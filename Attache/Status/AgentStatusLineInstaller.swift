...
enum AgentStatusLineInstaller {
    /// These three keep their `tmuxgui-` names for the reason spelled out on
    /// `AgentHookInstaller.scriptURL`: the path *is* the installed state. It is
    /// written into `statusLine.command` in `~/.claude/settings.json`, and
    /// `StatusLineRecovery` is what puts the user's own status line back — both
    /// match on this string. Rename the file and an already-wrapped status line
    /// can never be unwrapped again.
    ...
    # change. Remove it in Attaché (Settings -> Behaviour), or put the command
    # in that file back into ~/.claude/settings.json yourself.
    #
    # Nothing is parsed here. Attaché reads this JSON in Swift, so this script
    ...
    if [ -n "$TMUX_PANE" ]; then
        ...
        if mkdir "$lock" 2>/dev/null; then
            (
                ...
                # and a raw newline in an option value would break Attaché's
    ...
    #!/bin/sh
    # Attaché — a minimal Claude Code status line.
    #
