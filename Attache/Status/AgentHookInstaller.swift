...
enum AgentHookInstaller {
    ...
    /// Still `tmuxgui-`, and that is deliberate — see `isOurs`, which decides
    /// an entry in *another program's* settings belongs to this app by
    /// comparing this exact path. Renaming the file renames the path, and every
    /// hook already installed stops being recognised: the app can no longer
    /// remove what it put there, "install" adds a second copy beside the first,
    /// and there is no error anywhere to notice it by. Moving to an Attaché
    /// name means rewriting `~/.claude/settings.json` in place, which is its own
    /// change with its own way of going wrong, not a line in a rename.
