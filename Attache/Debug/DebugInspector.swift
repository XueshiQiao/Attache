...
//
    ...
    enum DebugInspector {
        ...
        static let snapshotURL = URL(fileURLWithPath: "/tmp/attache/inspect.json")
        ...
        static func screenshotBody(query: String) -> Data {
            let parameters = parseQuery(query)
            let path = parameters["path"] ?? "/tmp/attache/shot.png"
    ...
    extension DebugInspector {
        ...
        struct WindowNode: Encodable {
            ...
            init(offScreenRoot: NSView) {
                title = "Attaché Settings (built off screen)"
