...
enum TmuxLog {
    ...
    /// `~/Library/Logs/Attache/tmux-commands.log` — where macOS keeps app logs,
    /// and outside the app bundle so it survives a rebuild as well as a crash.
    static let fileURL: URL = FileManager.default
        .homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Logs/Attache", isDirectory: true)
    ...
    private static func emit(_ kind: Kind, session: String, caller: String, text: String) {
        ...
        // `Attache | head -1` — a completely ordinary thing to type — took the
    ...
    private static func openedHandle() -> FileHandle? {
        ...
        let banner = "\n=== Attaché \(ProcessInfo.processInfo.processIdentifier) "
