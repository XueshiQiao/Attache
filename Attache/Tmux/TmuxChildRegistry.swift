...
enum TmuxChildRegistry {
    ...
    static let fileURL: URL = FileManager.default
        ...
        .appendingPathComponent("Library/Application Support/Attache", isDirectory: true)
        .appendingPathComponent("control-clients")

    /// Where this file lived when the app was called TmuxGUI.
    ///
    /// A record here names a control client *this app* spawned, under its old
    /// name. The rename does not make that process any less this app's to
    /// collect, and an orphan is by definition the thing nothing else remembers
    /// — so a sweep that reads only the path above strands exactly what the file
    /// was written down to catch, and it keeps costing the user's tmux server
    /// memory with no sign of why.
    ///
    /// Read once during a sweep and then deleted. Its surviving records are
    /// carried into the file above, so a copy left behind would come back next
    /// launch as a duplicate of one already there. What it holds is pids, not
    /// anything of the user's.
    static let legacyFileURL: URL = FileManager.default
        .homeDirectoryForCurrentUser
    ...
    static func sweep() -> Int {
        ...
        var reclaimed = 0
        withFileLock {
            // Both files. See `legacyFileURL`: a run under the old name wrote
            // its children there, and they are still this app's to collect.
            let legacy = read(from: legacyFileURL)
            let records = read() + legacy
            ...
            let wrote = write(survivors)

            // Two conditions, and the file stays for another launch if either
            // fails. Deleting it is only safe once what it held is somewhere
            // else, and a `write` that failed on a full disk means it is not.
            //
            // The second condition is the cross-process one. This lock is the
            // sibling of the *current* file; a build still running under the old
            // name holds the sibling of the legacy one, and nothing makes those
            // two exclude each other. A child it records between the read above
            // and this line would be erased and never collected. Rather than
            // nest two locks and inherit an ordering to get wrong, leave the file
            // alone while any owner named in it is still alive — that run will
            // exit eventually, and the next sweep gets it.
            let legacyOwnerAlive = legacy.contains { isRunning($0.ownerPID) }
            if wrote, !legacyOwnerAlive {
    ...
    /// Caller holds `lock`.
    private static func read(from url: URL = fileURL) -> [TmuxChildRecord] {
        guard let text = try? String(contentsOf: url, encoding: .utf8) else { return [] }
        return text.split(separator: "\n").compactMap { TmuxChildRecord.parse(line: String($0)) }
    }

    /// Caller holds `lock`. Reports whether the file is now on disk, because one
    /// caller — the legacy sweep — must not throw the old copy away on the word
    /// of a write that did not happen.
    @discardableResult
    private static func write(_ records: [TmuxChildRecord]) -> Bool {
        try? FileManager.default.createDirectory(
            at: fileURL.deletingLastPathComponent(), withIntermediateDirectories: true
        )
        ...
        do {
            ...
            TmuxLog.lifecycle("could not write \(fileURL.path): \(error)", session: "-")
            return false
        }
    }
}
